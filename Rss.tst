...
Tst 03 Получение информации об активных объектах
	Rss -i \*
Tst W3 Восстановление измененных объектов
	echo >dir/file
	chmod 660 dir/file
	rm dir/link
	ln -sf file dir/link
	...
	Rss -nr \*
	Chk | diff -U0 .inf -
Tst W1 Восстановление файла вместо каталога
	rm dir/file
	mkdir dir/file
	Rss -nr dir/file
Tst W1 Восстановление файла вместо ссылки
	rm dir/file
	ln -sf bad dir/file
	Rss -nr dir/file
Tst W1 Восстановление ссылки вместо каталога
	rm dir/link
	mkdir dir/link
	Rss -nr dir/link
Tst W1 Восстановление ссылки вместо файла
	rm dir/link
	touch dir/link
	Rss -nr dir/link
Tst W1 Восстановление каталога вместо файла
	rm -rf dir
	touch dir
	Rss -nr dir
Tst W1 Восстановление каталога вместо ссылки
	rm -rf dir
	ln -s bad dir
	Rss -nr dir
Tst W3 Восстановление с нуля
	rm -rf dir
	Rss -nr \*
	Chk | diff -U0 .inf -
...
Tst 00 Сохранение объектов \(Version 2\)
	...
	Rss -b $LST
	Chk >.tmp
Tst W3 Восстановление на заданную дату
	Rss -D "$V1" -rn \*
	Chk | diff -U0 .inf -
Tst W3 Восстановление последних версий объектов
	Rss -rn \*
	Chk | diff -U0 .tmp -
Tst E2 Попытка восстановления файла в каталоге закрытом для записи
	...
	chmod 555 dir
	Rss -rn dir/file
Tst W1 Восстановлениe каталога закрытого для записи
	Rss -rn dir
...
Tst 05 Различия содержимого на заданную дату
	Rss -D "$D1" -d a
Tst W1 Удаляем самый старый файл
	Rss -D "$D0" -zn a
Tst 04 Различия содержимого на заданную дату
	Rss -D "$D1" -d a
Tst W1 Удаляем отметку удаления, где версии до и после удаления идентичны
	Rss -D "$D2" -zn a
Tst 05 Проверка наличия объектов в БД
	Rss -an a
Tst W1 Удаляем файл, где версии до и после него идентичны
	Rss -D "$D3" -zn a
Tst 03 Проверка наличия объектов в БД
	Rss -an a
Tst 05 Различия между предпоследней и последней версиями файла
	Rss -d a
Tst W1 Удаляем последнюю версию файла
	Rss -zn a
Tst 05 Различия между предпоследней и последней версиями файла
	Rss -d a
Tst W1 Оставляем единственный самый старый вариант
	Rss -zn a
Tst 04 Различия для единственного варианта
	Rss -d a
Tst W1 Окончательное удаление объекта
	Rss -zn a
