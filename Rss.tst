...
OUT="Test Reports/Restore shell script
	...
	@(#) Rss.tst V1.19 (C) 2007-2019 by Roman Oreshnikov
...
Tst 03 Получение информации об активных объектах
	Rss -i \*
Tst 07 Получение списка действий над объектами восстановления
	echo >"$WRK/dir/file"
	...
	chgrp -R 0 "$WRK/dir"
	Rss -t \*
Tst 03 Получение списка объектов восстановления
	Rss -R . -nt \*
...
Tst E1 Обновление объекта для записи с ошибкой в БД
	Rss -b -R "$WRK/dir/none" "$LOG"
Tst 00 Сохранение файла без возможности восстановления наполнения
	Rss -nb -R "$WRK/dir/none" "$TMP"
Tst W1 Попытка восстановления измененного файла с несохраненным наполнением
	echo Fi >"$WRK/dir/none"
	Rss -rn "$WRK/dir/none"
Tst 00 Восстановление прав доступа к файлу с несохраненным наполнением
	cp "$TMP" "$WRK/dir/none"
	chmod 444 "$WRK/dir/none"
	Rss -rn "$WRK/dir/none"
...
Tst 09 Запуск Rss в режиме отчета для каталога c примерами отчетов
	Rss -s "$WRK/dir/rep"
Tst 012 Формирование тестового отчета с ошибками вызова встроенной функции
	cat <<-END >"$REP"
	Rss "$TST/*"
	Rss "$TST/./dir"
	Rss "$TST/../dir"
	...
	Rss "$TST//dir"
	Rss "$TST/ /dir"
	Rss TST/dir/ Групповые операции разрешены только с абсолютным именем
	Rss "$TST/dir/" + a - b
	Rss "$TST/dir" =
	...
	echo "$ cat $REP"
	cat "$REP"
Tst 033 Запуск Rss в режиме отчета для тестового отчета с ошибками вызова
	Rss -s "$REP"
...
Tst 024 Получение тестового отчета повторно
	sleep 1
	Rss -s "$REP"
Tst 020 Формирование тестового отчета работы с файлами
	mkdir -p "$WRK/dir"
	echo >"$WRK/dir/file"
	...
	# Предварительное конвертирование файла
	Rss "$TST/dir/file1" @ Edit
	# Нестандартный поиск отличий
	Rss "$TST/dir/file2" = Diff
	# Предобработка и свое сравнение
	Rss "$TST/dir/file3" @ Ed = Di
	END
	chmod 755 "$REP"
	echo "$ cat $REP"
	cat "$REP"
...
Tst 00 Получение тестового отчета работы с файлами повторно
	sleep 1
	Rss -s "$REP"
Tst 027 Получение тестового отчета работы с файлами после изменений
	rm "$WRK/dir/file"
...
Tst 016 Формирование тестового отчета работы со списками файлов
	...
	cat <<-END >"$REP"
	#!/bin/sh
	Rss $TST/dir/ @ false Контент файлов не сохраняется
	Rss $TST/dir/ - rep
	END
	chmod 755 "$REP"
	echo "$ cat $REP"
	cat "$REP"
	V3=$(date)
Tst 08 Получение тестового отчета работы со списком файлов
	sleep 1
	Rss -s "$REP"
...
Tst 012 Получение всех версий файлов
	Rss -R "$WRK/0" -x \* && ls "$WRK/0$TST/dir"
Tst 08 Получение всех версий файлов до даты
	Rss -R "$WRK/1" -D "$V3" -x \* && ls "$WRK/1$TST/dir"
Tst 04 Получение всех версий файлов старше даты
	Rss -R "$WRK/2" -D "$V3" -nx \* && ls "$WRK/2$TST/dir"
Tst W1 Получение всех версий файлов старше текущей даты
	Rss -R "$WRK/3" -nx \* && ls "$WRK/3$TST/dir"
