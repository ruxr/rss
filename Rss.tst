...
OUT="Test Reports/Restore shell script
	...
	@(#) Rss.tst V1.19.1 (C) 2007-2019 by Roman Oreshnikov
...
Tst 09 Запуск Rss в режиме отчета для каталога c примерами отчетов
	Rss -s "$WRK/dir/rep"
Tst 011 Формирование тестового отчета с ошибками вызова встроенной функции
	cat <<-END >"$REP"
	Rss "$TST/*"
	Rss "$TST/./dir"
	Rss "$TST/../dir"
	...
	echo "$ cat $REP"
	cat "$REP"
Tst 030 Запуск Rss в режиме отчета для тестового отчета с ошибками вызова
	Rss -s "$REP"
...
Tst 06 Получение всех версий файлов до даты
	Rss -R "$WRK/1" -D "$V3" -x \* && ls "$WRK/1$TST/dir"
Tst 06 Получение всех версий файлов старше даты
